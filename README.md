...
#### 配置文件系统 ⭐test分支
- **settings.json**: 集中式配置文件，所有游戏参数可配置
  - 游戏设置: 分辨率、帧率、速度、伤害等
  - 网络设置: 端口、超时、心跳间隔等
  - AI设置: 使用增强版AI开关
  - 命令设置: 命令前缀、启用/禁用
  - 颜色定义: 所有游戏颜色
- **config.py**: 配置加载模块，支持动态获取和修改
- **向后兼容**: constants.py 保持原有API不变

#### 指令系统重构 ⭐test分支
- **独立模块**: `game_commands.py` 统一管理系统
- **命令格式**: 所有命令使用 `.` 前缀（如 `.addai`）
- **权限控制**: 区分普通玩家、服务器、管理员权限
- **子命令支持**: `.team add/delete/list/join/leave`
- **AI性格隐性**: 性格由系统随机生成，玩家无法查看或选择
- **AI名称简化**: 显示为 `AI_{ID}`，不包含性格信息
- **帮助系统**: `.help` 显示分类帮助，`.help <命令>` 显示详细说明
...
| | `.list` / `.players` | 列出玩家 |
| | `.listai` | 列出AI（不显示性格） |
| 聊天 | `.teamchat` / `.tc` | 团队聊天模式 |
| | `.all` | 全局聊天模式 |
| AI管理 | `.addai [难度]` / `.spawn` | 添加AI（难度: easy/normal/hard） |
| | `.removeai <ID\|all>` / `.delete` | 移除AI |
| 团队管理 | `.team add [名称]` | 创建团队 |
| | `.team delete <ID>` | 删除团队 |
| | `.team list` | 列出团队 |
| | `.team join <ID>` | 加入团队 |
| | `.team leave` | 离开团队 |
