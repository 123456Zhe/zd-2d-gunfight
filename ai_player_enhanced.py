...
class EnhancedAIPlayer:
    ...
    def update_pathfinding(self, target_pos):
        ...
        # 检查是否需要重新规划路径
        need_repath = (
            current_time - self.last_pathfind_time > self.pathfind_interval
            ...
            if direction.length() > 0:
                ...
                if game_map:
                    if self.can_move_in_direction(
                        direction_normalized, game_map, distance=30
                    ):
                        # 方向安全，返回它
    ...
    def update_stealth_mode(self, players, game_map):
        ...
        # 根据个性化特征判断
        should_stealth = self.personality_traits.should_use_stealth(
            min_distance, len(enemies)
        )
